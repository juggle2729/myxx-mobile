...
.level-help-view.bg-default
    .user.center.font-30.bg-white(v-if="isUserView")
        ...
        .bar-label.flex
            ...
            span 店铺积分
            .hr.flex-1
        .lv-bar.font-22.white(:style="barStyle") {{point}}/{{point+gap}}
        .red 还差{{gap}}点积分升级到{{lv+1}}级
    .separator-20-no(v-if="isUserView")
    .lvs.bg-white
        header.font-30.border-gray 积分等级制度
        .item.flex.font-30(v-for="(i, point) in lvs")
            lv(:lv="i+1")
            .flex-1 {{i+1}}级
            div {{point}}分
    .separator-20-no
    .usage.font-30.bg-white(v-if="isUserView")
        h3.gray 店铺等级有什么用？
        ...
        .flex
            label 商品审核通过
            span +10分
    .desc.font-30.gray(v-else) 店铺等级受销售额、好评率和退货率影响。等级越高，信誉和商品质量越有保障。
</template>
<script>
