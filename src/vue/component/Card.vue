...
<div class="card-component bg-white">
    <div v-if="type === 'jb'" v-link="{name: 'evaluation', params: {id: entry.id, result: 'none'}}">
        ...
        <div class="data-info fz-26"><div class="data-name line-clamp-2">{{entry.description}}</div></div>
    </div>
    <div v-else v-link="{name: 'story', params: {id: entry.post_id}}">
        <div class="data-label fz-22 center white">{{entry.topic_type.name}}</div>
        ...
        <div class="data-info fz-26"><div class="data-name line-clamp-2">{{entry.content}}</div></div>
    </div>
