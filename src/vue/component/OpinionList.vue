...
.opinion-list.bg-white(:class="{ 'pdh-36': !items.isEmpty }")
    .opinion.flex.pdt-36.pdb-20.bdb(v-for="item in items")
        ...
        template(v-else)
            img.anonymous(:src="'shop/anonymous.png' | qn")
        .mgl.flex-1(@click="go(item)")
            ...
            .fz-30.user-txt(:class="{'mgt-22': item.description}") {{ item.description }}
            .pdv-32.pdl-28.fz-26.black-47.user-txt.bg-gray-f7.dark.mgt-22(v-if="item.reply") 商家回复： {{{ item.reply.reply_content | content }}}
            ...
            .product.pdv-10.bd.mgt-22.flex(v-if="!isChat")
                ...
                .title.fz-20.dark-6b.mgl-20.line-clamp-1 {{ item.product_title }}
